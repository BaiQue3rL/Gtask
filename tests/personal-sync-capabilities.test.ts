...
describe('个人进度同步能力', () => {
  it('只开放个人接口实际提供的版块', () => {
    ...
    expect(getPersonalSyncTargets('star-rail')).toEqual(['cycles'])
    expect(getPersonalSyncTargets('zenless')).toEqual(['events', 'cycles'])
    expect(getPersonalSyncTargets('wuthering-waves')).toEqual([])
    expect(supportsPersonalSyncTarget('star-rail', 'exploration')).toBe(false)
    expect(supportsPersonalSyncTarget('star-rail', 'events')).toBe(false)
