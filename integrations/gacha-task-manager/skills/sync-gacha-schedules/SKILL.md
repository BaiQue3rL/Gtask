...
- Before submitting an events refresh, inspect the existing active limited-event checklist. Treat every existing event with no `activityTags`, with the legacy tag `待识别`, or with the fallback tag `未知`, as an enrichment target even when it came from personal data or an older sync. Research it and resubmit the same activity identity with verified tags while preserving its existing completion state. A refresh is incomplete if a currently active event that can be identified is left untagged or left as `未知` without a fresh cross-check.
