...
export function parsePublicScheduleDocument(
  ...
  if (!isRecord(value) || value.schemaVersion !== 1) throw new Error('公开资料文档版本不受支持')
  const gameId = parseGameId(value.gameId)
  if (gameId !== expectedGameId) throw new Error('公开资料文档与请求游戏不匹配')
  const sourceUrl = parseExternalUrl(value.sourceUrl)
  if (typeof value.fetchedAt !== 'string' || Number.isNaN(Date.parse(value.fetchedAt))) {
    throw new Error('公开资料抓取时间格式不正确')
  }
  if (!Array.isArray(value.items)) throw new Error('公开资料事项列表格式不正确')
  if (value.items.length > MAXIMUM_DOCUMENT_ITEMS) {
    throw new Error(`公开资料事项不能超过 ${MAXIMUM_DOCUMENT_ITEMS} 条`)
  }

  const items: NormalizedSyncItem[] = value.items.map((candidate): NormalizedSyncItem => {
    if (!isRecord(candidate)) throw new Error('公开资料事项格式不正确')
    ...
    return {
      ...normalized,
      scheduleKind: ['main_quest', 'side_quest', 'limited_event'].includes(normalized.category)
        ? 'fixed_window'
...
export class PublicScheduleDocumentAdapter implements SyncAdapter {
  ...
  async sync(gameId: GameId): Promise<SyncAdapterOutput> {
    ...
    if (ageMs > this.maximumAgeMs) throw new Error('公开资料文档已过期')
    if (ageMs < -this.maximumFutureSkewMs) throw new Error('公开资料抓取时间异常地来自未来')
    return {
      items: document.items,
      message: `公开资料已同步，来源抓取于 ${document.fetchedAt}`
    }
  }
}
