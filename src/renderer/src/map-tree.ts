import type { ChecklistItem } from '../../shared/contracts'
import { compareMapTreeItems } from './checklist-sort'
import { isChecklistItemComplete } from './checklist-completion'

export interface ChecklistTreeRow {
  item: ChecklistItem
  // Presentation identity can differ while a self row edits the original item.
  presentationKey?: string
  depth: number
  hasChildren: boolean
  displayProgressPercent: number | null
  mapSummary?: { completed: number; total: number; unknown: number }
}

export function checklistRowKey(row: ChecklistTreeRow): string {
  return row.presentationKey ?? row.item.id
}

function mapLeafProgress(item: ChecklistItem): number | null {
  return item.reportedProgressPercent === null && !item.manualCompletionLocked
    ? null : item.progressPercent
}

function mapLeafComplete(item: ChecklistItem): boolean {
  if (item.reportedProgressPercent === null && !item.manualCompletionLocked) return false
  return isChecklistItemComplete(item)
}

export function isChecklistRowComplete(row: ChecklistTreeRow): boolean {
  return row.mapSummary
    ? row.mapSummary.total > 0 && row.mapSummary.completed === row.mapSummary.total
    : row.item.category === 'exploration' ? mapLeafComplete(row.item) : isChecklistItemComplete(row.item)
}

export function collectMapBranchKeys(items: ChecklistItem[]): Set<string> {
  return new Set(
    buildMapTreeRows(items, new Set(), items)
      .filter((row) => row.hasChildren)
      .map((row) => row.item.remoteKey ?? row.item.id)
  )
}

export function buildMapTreeRows(
  sourceItems: ChecklistItem[],
  collapsedKeys: ReadonlySet<string>,
  progressSourceItems: ChecklistItem[] = sourceItems,
  now = Date.now()
): ChecklistTreeRow[] {
  const isOpen = (item: ChecklistItem): boolean => !item.startsAt || Date.parse(item.startsAt) <= now
  const items = sourceItems.filter(isOpen)
  const progressItems = progressSourceItems.filter(isOpen)
  const keyOf = (item: ChecklistItem): string => item.remoteKey ?? item.id
  const byKey = new Map(progressItems.map((item) => [keyOf(item), item]))
  const titleGroups = new Map<string, ChecklistItem[]>()
  for (const item of progressItems) {
    const group = titleGroups.get(item.title) ?? []
    group.push(item)
    titleGroups.set(item.title, group)
  }
  const parentKeyOf = (item: ChecklistItem): string | null => {
    if (item.parentRemoteKey && byKey.has(item.parentRemoteKey)) return item.parentRemoteKey
    if (!item.parentTitle) return null
    const candidates = titleGroups.get(item.parentTitle) ?? []
    return candidates.length === 1 ? keyOf(candidates[0]) : null
  }
  const children = new Map<string, ChecklistItem[]>()
  for (const item of progressItems) {
    const parentKey = parentKeyOf(item)
    if (!parentKey || parentKey === keyOf(item)) continue
    const group = children.get(parentKey) ?? []
    group.push(item)
    children.set(parentKey, group)
  }
  const visibleKeys = new Set(items.map(keyOf))
  // A filter may hide a completed parent while leaving unfinished children
  // visible. Preserve the necessary ancestor path as structure instead of
  // promoting those children into fake root regions.
  const renderKeys = new Set(visibleKeys)
  for (const visible of items) {
    let current: ChecklistItem | undefined = visible
    const visitedAncestors = new Set<string>()
    while (current) {
      const key = keyOf(current)
      if (visitedAncestors.has(key)) break
      visitedAncestors.add(key)
      renderKeys.add(key)
      const parentKey = parentKeyOf(current)
      if (!parentKey || parentKey === key) break
      current = byKey.get(parentKey)
    }
  }
  const summaries = new Map<string, NonNullable<ChecklistTreeRow['mapSummary']>>()
  for (const [key, descendants] of children) {
    if (byKey.get(key)?.mapNodeKind !== 'region') continue
    summaries.set(key, {
      completed: descendants.filter(mapLeafComplete).length,
      total: descendants.length,
      unknown: descendants.filter(child => !mapLeafComplete(child) && mapLeafProgress(child) === null).length
    })
  }
  const catalogParentKeys = new Set(progressSourceItems.map(parentKeyOf).filter(Boolean))
  const selfGroups = new Set<string>()
  for (const item of progressItems) {
    const key = keyOf(item)
    if (item.mapNodeKind !== 'region' || summaries.has(key)) continue
    const hasCatalogChildren = catalogParentKeys.has(key)
    if (!hasCatalogChildren) selfGroups.add(key)
    summaries.set(key, {
      completed: !hasCatalogChildren && mapLeafComplete(item) ? 1 : 0,
      total: hasCatalogChildren ? 0 : 1,
      unknown: !hasCatalogChildren && !mapLeafComplete(item) && mapLeafProgress(item) === null ? 1 : 0
    })
  }
  const complete = (item: ChecklistItem): boolean => {
    const summary = summaries.get(keyOf(item))
    return summary ? summary.total > 0 && summary.completed === summary.total : mapLeafComplete(item)
  }
  const compare = (left: ChecklistItem, right: ChecklistItem): number =>
    Number(complete(left)) - Number(complete(right)) ||
    compareMapTreeItems({ ...left, completed: false }, { ...right, completed: false })
  progressItems.sort(compare)
  for (const descendants of children.values()) descendants.sort(compare)
  const roots = progressItems.filter((item) => {
    if (!renderKeys.has(keyOf(item))) return false
    const parentKey = parentKeyOf(item)
    return !parentKey || parentKey === keyOf(item)
  })
  const rows: ChecklistTreeRow[] = []
  const visited = new Set<string>()
  const reachable = new Set<string>()
  const markReachable = (item: ChecklistItem): void => {
    const key = keyOf(item)
    if (reachable.has(key)) return
    reachable.add(key)
    for (const child of children.get(key) ?? []) {
      if (renderKeys.has(keyOf(child))) markReachable(child)
    }
  }
  const visit = (item: ChecklistItem, depth: number): void => {
    const key = keyOf(item)
    if (visited.has(key)) return
    visited.add(key)
    const descendants = children.get(key) ?? []
    const visibleDescendants = descendants.filter((child) => renderKeys.has(keyOf(child)))
    rows.push({
      item,
      depth,
      hasChildren: visibleDescendants.length > 0 || selfGroups.has(key),
      displayProgressPercent: summaries.has(key) ? null : mapLeafProgress(item),
      ...(summaries.has(key) ? { mapSummary: summaries.get(key)! } : {})
    })
    if (collapsedKeys.has(key)) return
    if (selfGroups.has(key)) {
      rows.push({ item, presentationKey: `self:${item.id}`, depth: depth + 1,
        hasChildren: false, displayProgressPercent: mapLeafProgress(item) })
    }
    for (const child of visibleDescendants) visit(child, depth + 1)
  }
  for (const root of roots) markReachable(root)
  for (const root of roots) visit(root, 0)
  for (const item of progressItems) {
    if (renderKeys.has(keyOf(item)) && !reachable.has(keyOf(item))) visit(item, 0)
  }
  return rows
}

export function filterIncompleteMapTreeRows(rows: ChecklistTreeRow[]): ChecklistTreeRow[] {
  return rows.filter((row) => !isChecklistRowComplete(row))
}

export function distributeMapTreeRows(
  rows: ChecklistTreeRow[],
  columnCount = 2
): ChecklistTreeRow[][] {
  const columns = Array.from(
    { length: Math.max(1, Math.floor(columnCount)) },
    (): ChecklistTreeRow[] => []
  )
  const branches: ChecklistTreeRow[][] = []
  for (const row of rows) {
    if (row.depth === 0 || branches.length === 0) branches.push([])
    branches.at(-1)!.push(row)
  }
  for (const branch of branches) {
    const target = columns.reduce(
      (shortest, column) => column.length < shortest.length ? column : shortest,
      columns[0]
    )
    target.push(...branch)
  }
  return columns
}
