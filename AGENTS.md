...
- For map progress, a region value from the current personal snapshot is authoritative and must persist across restarts and public catalog refreshes. Only when that snapshot omits the region value may the runtime derive the region average from matched canonical subregions; historical personal bindings alone must not suppress that fallback.
