...
- 地图接口只更新匹配行的进度事实，不能生成目录或改变父子关系。有二级目录的一级地区是清单分组：完成和筛选按已收录、已开放子区的完成数决定，与接口总百分比分开。接口一级值仍原样保留供参考；旧库缺一级值时的派生平均仅保留兼容，不能显示成官方值或决定分组完成。查询返回 `reportedProgressPercent` 区分有个人证据的值与派生/未观察值，不迁移或重置玩家数据。
...
- 提前显示开关统一影响所有游戏的公共限时活动和周期挑战；关闭时隐藏开始时间尚未到达的事项（含周期空窗期的下一期），到点自动显示。开启时保留未来事项并显示距离开始的时间。地图清单只收录已开放地图，不提供未来地图预览，不受此开关影响；手动/自定义事项也不受影响。该设置不修改基准或个人进度。界面加载以最新请求为准，过期响应不得覆盖当前游戏或已完成的操作。
...
- “只看未完成”依据子区汇总隐藏整组，保留未完成或未知子区所属的一级分组与缩进；收起分组不能让子区变成顶层项。一级主显示只有 `x/x`，悬停解释收录范围、未知数量及有证据时的官方总探索度。至冬官方 52% 而 5/5 子区完成时可整组隐藏；莱姆尼安官方 100% 而只有 7/13 完成时分组保留。一级分组只能展开/折叠，不提供整组完成操作，IPC 同样拒绝这类写入。无二级目录时按有证据的个人进度或手动状态显示。此规则取代此前隐藏父项、提升子项的规则（用户授权，2026-09-15）。
- 子区分母来自当前有效公共清单，明确未来开放的条目暂不计入；未知进度不能当完成，失败或缺项保留已有可靠进度。新增已开放子区自动使分组重新出现，保留原有子区状态。分组汇总不覆盖数据库中的接口原值，不宣称覆盖未收录地图。
- 没有收录子区的一级地图（如空之神殿）也显示 `0/1` 或 `1/1`，展开后呈现同名二级行，二级百分比和手动操作直接使用原有记录。父子行有不同的界面标识，虚拟滚动、行高缓存和筛选不会相互覆盖；数据库和个人绑定不增加重复项。后续正式收录子区时改为真实子区，不从原父级百分比推断新子区完成。只有未来子区时暂显示 `0/0`，不生成无法安全操作的同名行。
- 无子区地图在后续个人快照漏项时保留历史可靠进度。兼容逻辑派生父级平均时清除其旧快照引用，避免将派生值误认成个人接口原值；原始历史快照按既有保留策略管理。
