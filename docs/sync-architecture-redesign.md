...
状态：双入口数据面已实施，等待从零真实账号回归
确认日期：2026-07-27；首次实施：2026-07-28（Asia/Shanghai）
...
- “同步清单”和“同步进度”都可增量建立同一份规范目录：公开资料建立完整目录，个人数据可在空目录中先建立已观察到的局部目录。
- 每个游戏/版块保存 `empty / partial / complete` 目录覆盖度；个人数据只能建立或维持 `partial`，已由公开资料确认的 `complete` 不会被后续个人增量降级。
...
- Gtask MCP 协议增加版本握手和同批次语义候选批量领取；地图首次建表不再为每条记录启动独立流程。
- 公开资料后到时必须复用个人数据已建立的规范项目 ID，并补全名称、分类、时间和层级；来源绑定、个人状态和手动锁不得被替换。
- 自动回归数量以本轮最终运行结果为准。
...
当前四款游戏的个人适配器几乎都返回 `items: []` 和 `reviewCandidates: [...]`。因此当前真实路径是：
...
协调器仍保留直接合并 `items` 的机械快路径，但当前个人适配器基本没有使用它。
...
- 第一次个人同步可能需要 Codex 批量建表和绑定；同一来源身份第二次开始通常走机械快路径。
- 两个入口都允许增量新增。个人入口只代表“已观察到这些项目”，不能宣称整个版块目录完整；公开入口负责确认完整覆盖。
