...
状态：规范清单优先的数据面已实施；地图两级基准目录与退出取消链路已完成，等待从零真实账号回归
确认日期：2026-07-27；首次实施：2026-07-28；地图两级目录：2026-07-31（Asia/Shanghai）

> 本文是底层同步重构的唯一接续基准。实施期间不得清空真实数据库、凭据或手动状态。

## 实施快照（2026-07-28）

- SQLite 已升级到 v24，新增并持续使用 `source_bindings`、`personal_item_states`、`semantic_profiles`、`sync_observations`，原清单表继续作为兼容的规范目录；来源绑定可保存活动状态的可复用字段规则。
...
- 四游戏周期挑战按已验证的 `modeKey` 和“存在挑战记录”语义直接写入；官方接口同时给出模式、周期和完整时间窗时，缺失的当期项目也可机械增量建立。地图严格只保留一级主地区 `region` 与归属唯一主地区的二级地区 `subregion`。内置规范目录先建表；登录后的官方接口若同时给出稳定 ID、本地化名称、0–100 探索度及结构化父子关系，可机械绑定或增量补项。只有 ID 冲突、父级缺失、结构矛盾和其他不明确情况进入 Codex。
...
- 语义候选指纹升级到 `codex-authority-v8` 并纳入账号作用域；旧拒绝项可重新入队，历史已通过结论可回填。
- Gtask MCP 协议增加版本握手和同批次语义候选批量领取；个人候选领取增加规范目录完成门槛。
- 地图公开资料任务把当前 `matchCandidates` 视为完整基准，只提交新地区、官方改名或父级归属修正；没有增量时允许空提交完成。普通本地合并时间不冒充 Codex 核验时间。
- 个人数据不会用模糊标题或推测层级反向建表；但经过认证的官方接口若提供完整且结构化的确定事实，可以安全增量补齐地图或周期目录。个人状态和手动锁始终独立保存。
- 自动回归数量以本轮最终运行结果为准。
...
- 公开资料同步由 Codex 联网检索活动、周期和版本；地图先使用经资料与用户截图校对的内置规范目录。地图按钮结合内置目录核验时间、最近一次成功的 Codex 地图任务和任务版块版本窗口判断新鲜度：同一版本内直接恢复基准，跨版本或无版本窗口时超过 30 天才触发一次 Codex 增量核验。
- 米游社、库街区个人适配器负责登录并采集官方个人数据。
- 语义审核队列只把无法机械唯一匹配、出现改名/冲突或字段语义未知的个人候选交给 Codex。
...
→ 公开规范清单 complete
→ 已有来源绑定与字段语义：机械写入
→ 官方确定字段可唯一建立新绑定/新项目：机械增量写入
→ 冲突、结构不完整或未知语义：Codex 异常审核
→ SQLite
```
...
- 原神、绝区零、鸣潮地图探索度在规范目录绑定后走本地确定性解析。
- 星铁没有可用的个人地图百分比时保持规范目录不变。
...
flowchart TD
    ...
    subgraph Catalog["同一规范目录"]
        ...
        M -->|能| MB["机械建立绑定"]
        M -->|不能| T{"官方接口事实足以唯一增量建项？"}
        T -->|是| TI["机械增量建项并保存绑定"]
        T -->|否| X["小批次进入 Codex 异常队列"]
...
- 两个入口都允许用户发起增量更新，但只有公开资料流程能声明目录完整。个人接口发现的新 ID 若带有经过适配器声明的官方身份、本地化名称、数值和结构事实，可机械增量补入；任何缺项、冲突或业务分类不明确都进入 Codex，而不是猜测。
