...
后台 MCP 可排队 `public_catalog` 维护任务，核验活动、周期、地图或版本窗口。核验后的公共增量可发布到 `updates/catalog.json`：客户端启动时按设置检查 Gitee/GitHub，严格校验协议后在一个数据库事务中更新 `public_schedule`。远程清单不能提交完成状态、探索度、凭据或 `custom` 项；任一游戏或地图层级校验失败时整批回滚，继续使用上一次持久基准。旧镜像不能覆盖更新的本地发布时间。大型结构、适配器或程序变更仍通过软件版本发布。
