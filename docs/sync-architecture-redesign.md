...
- 正常个人同步不等待 Codex、不等待公开清单、不做名称相似度匹配。完整快照先原子激活；仅缺失的玩法标签与活动/周期时间可在写入后进入独立 Codex 元数据旁路。
...
- 公开资料 Codex Worker + MCP：联网检索并提交公开清单；另有严格受限的 `personal_metadata` 任务只补既有个人事项的标签与时间。
...
flowchart TD
    ...
    A4 -->|失败| K
    A4 -->|通过| A5["机械淘汰已到期身份并单事务替换为 personal_sync"]
...
- 仍在回收站中的项目是用户明确暂时隐藏的项目：同步只更新当前列表，不能恢复、更新或删除回收站项目。用户清空回收站后，下一次完整个人快照允许按官方数据重新建立这些项目，作为误删恢复兜底。
...
- Codex 是公开资料的业务语义权威，负责联网检索、分类、名称、时间、标签和公开目录的增删改；个人元数据任务只允许更新契约点名的标签/时间字段。
