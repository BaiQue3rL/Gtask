...
启动时种子以 `verifiedAt` 比较新旧。包内表只覆盖更旧数据，不能把后台维护得到的新表倒退。活动合并后淘汰已到期系统项；周期到期时原位滚动到当前期并清除旧期完成状态；地图保持稳定键和两级结构。

后台 MCP 可排队 `public_catalog` 维护任务，热更新活动、周期、地图或版本窗口。它不出现在普通用户界面，不是启动或个人同步的依赖，也不能读取账号凭据或修改个人完成状态。大型结构、适配器或程序变更仍通过软件版本发布。
...
- 活动日历状态不等于“活动全部完成”。没有已验证完成字段时只用于匹配，不覆盖完成状态。
...
- 数据库 schema 当前为 v3。v2 → v3 会先保留旧个人行的完成/进度并吸收到完整基准，再删除旧个人结构。
