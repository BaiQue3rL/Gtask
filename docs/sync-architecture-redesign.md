...
- 四游戏周期挑战按已验证的 `modeKey` 和“存在挑战记录”语义直接写入；地图严格只保留一级主地区 `region` 与归属唯一主地区的二级地区 `subregion`。内置规范目录先建表，个人数据再按官方 ID 或规范目录内唯一的正式名称建立映射；来源层级只作观测，不覆盖规范父子关系。绑定完成后直接写入百分比。
