...
本轮修复四游戏“提前显示”：移除周期挑战的绕过逻辑，公共限时活动和周期挑战统一服从开关。用户明确地图只应收录当下已开放目录，不提供未来地图预览；地图和自定义事项不受该开关影响。467 项测试、类型检查、生产构建、清单消费校验与隔离 Electron 四游戏开关/重启验收通过。用户授权提交代码与清单并推送，明确不更新版本号；本轮不发布 Release、不替换正式安装。

公共清单 revision 为 `2026-09-24.genshin-7.1-baseline-maintenance`，发布 3 项兼容活动；另外 3 项已在本机写入，但依赖本机自定义标签，现有公共协议不能分发其定义，暂保留于仓库研究记录。完整事实与公开发布范围见 `catalog-research/genshin-7.1-2026-09-24.json`，修复及验证见 `upcoming-visibility-fix-2026-09-24.md`。不得将旧部署授权用于本轮自动覆盖安装。
