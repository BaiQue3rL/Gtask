...
- 远程清单消费兼容 v1/v2。首次发布时 `updates/catalog.json` 为 v1，revision 为 `2026-09-14.zenless-3.2-baseline-maintenance`；该轮没有为了软件审计捏造或发布新的游戏排期，当前修订见上方“当前工作”。
...
2026-09-15 历史部署备份记录：`D:\Git\Gtask-backups\20260915-210454-pre-map-summary-1.2.0`，当时包含旧程序、SQLite 在线备份、设置及加密凭据。该次安装后 app.asar SHA-256 为 `41f479a4cea76524050441e15fc499e4975c6526b459578eed534bd97c0753df`；历史回执见该目录的 `deployment.json` 和 `pre-deployment.json`。当时记录保留了 20:48 的备份和 18:13、18:26、18:42 的数据备份，后三者的旧 `program` 已按此前授权移入回收站。最新本机部署见上方“当前工作”；回退需考虑数据库 schema 8，不得擅自覆盖人工核验期间新增的数据。
